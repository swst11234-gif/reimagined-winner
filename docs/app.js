...
function normalizeImagePath(path) {
  if (!path) return "assets/placeholder-1.svg";
  const normalized = String(path).trim().replace(/\\/g, "/");
  if (!normalized) return "assets/placeholder-1.svg";
  return encodeURI(normalized);
}

function setImageWithFallback(img, imagePath, fallback = "assets/placeholder-1.svg") {
  const fallbackSrc = normalizeImagePath(fallback);
  img.src = normalizeImagePath(imagePath || fallbackSrc);
  img.onerror = () => {
...
function createCard(product) {
  ...
  setImageWithFallback(image, product.images?.[0], "assets/placeholder-1.svg");
...
function openModal(productId) {
  ...
  (product.images || ["assets/placeholder-1.svg"]).forEach((image) => {
    ...
    setImageWithFallback(img, image, "assets/placeholder-1.svg");
