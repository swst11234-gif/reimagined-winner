...
const normalizeImagePath = (p) => {
  const raw = (p || PLACEHOLDER_IMAGE).trim().replace(/\\/g, "/");
  const withoutDocsPrefix = raw.replace(/^\.?\/?docs\//i, "");
  const normalized = withoutDocsPrefix.startsWith("/") ? withoutDocsPrefix.slice(1) : withoutDocsPrefix;
  return encodeURI(normalized || PLACEHOLDER_IMAGE);
};
...
function resolveImage(product, selection) {
  const base = product.imageBase || product.images?.[0] || product.image || PLACEHOLDER_IMAGE;
